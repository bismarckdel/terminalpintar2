  ...
  <div class="min-h-screen bg-gray-100">
    ...
    <main class="p-8 max-w-3xl mx-auto">
      <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-green-600">Edit Buku</h1>
        <p class="text-gray-600 text-sm mt-2">Perbarui informasi buku yang sudah ada</p>
      </div>
        ...
        <form @submit.prevent="submitForm">
          <div class="space-y-6">
            ...
            <div>
              ...
              <p v-if="buku.status === 'Dipinjam'" class="mt-1 text-sm text-yellow-600">
                ⚠️ Buku ini sedang dipinjam. Pastikan sudah dikembalikan sebelum mengubah status.
              </p>
            </div>

            <!-- Info Peminjaman (jika sedang dipinjam) -->
            <div v-if="buku.status === 'Dipinjam'" class="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div class="flex items-start gap-2">
                <svg class="w-5 h-5 text-blue-600 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                </svg>
                <div>
                  <p class="text-sm font-medium text-blue-800">Informasi Peminjaman</p>
                  <p class="text-xs text-blue-600 mt-1">
                    Status buku saat ini: <span class="font-semibold">{{ buku.status }}</span>
                  </p>
                  <p class="text-xs text-blue-600 mt-1">
                    Gunakan menu Sirkulasi untuk proses pengembalian buku.
                  </p>
                </div>
              </div>
            </div>
...
const form = useForm({
  ...
  kategori: props.buku.kategori || '',
  status: props.buku.status
});

const submitForm = () => {
  form.post(`/admin/perpustakaan/${props.buku.id}/update`, {
    preserveScroll: true
  });
};
</script>
